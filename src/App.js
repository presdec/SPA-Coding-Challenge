...
function App() {
  ...
  useEffect(() => {
    fetch('https://freecurrencyapi.net/api/v2/latest?apikey=9fc011a0-86af-11ec-8d59-8560df2ac44e')
      ...
      .then(res => {
        ...
        // convert base from USD to EUR as freecurrencyapi.net has a higher limit on free USD fetches
        //"https://freecurrencyapi.net/api/v2/latest?apikey=fda125e0-876e-11ec-a152-454682c54a96&base_currency=EUR"
        // Could have been used which would allow a const displayCurrency = "EUR", To ensure working status i've opted
        // for this conversion instead, which displays a simple API fetch rather than a static JSON rates file.
        const ratio = parseFloat(currencies.EUR);
        const rates = {};

        Object.keys(currencies).forEach(key => {
          rates[key] = {
            EUR: { amount: parseInt( ratio * 100/ (currencies[key])), scale: 2 }
          }
        })
        rates['USD'] = {
          EUR: { amount: parseInt(ratio * 100) , scale: 2 }
        }
  ...
  const firstChildColumns = [
    { key: 'portfolioName', name: 'Portfolio #' },
  ...
  ({
    ...
    netWorth: intlFormat(100 * getCustomerAggregatedNetWorth(el)),
    restrictions: getCustomerAggregatedRestrictions(el),
    capitalGain: intlFormat(100 * getCustomerAggregatedCapitalGain(el)),
    ...
    ({
      ...
      assets: portfolio.assets.map(asset => (
        {
          ...asset,
          valuePerAsset: intlFormat(100 * convertToEURO(parseInt(asset.valuePerAsset), asset.currency)),
          capitalGainPerAsset: intlFormat(100 * convertToEURO(parseInt(asset.capitalGainPerAsset), asset.currency)),
          associatedRiskPerAsset: intlFormat(100 * convertToEURO(parseInt(asset.associatedRiskPerAsset), asset.currency)),
        })
      )
    }
    ))
